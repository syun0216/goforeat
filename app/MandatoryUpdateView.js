...
export default class MandatoryUpdateView extends Component {
    ...
    return (
      <Container>
        ...
        >
          ...
          {this.state.viewStatus == ViewStatus.VIEW_STATUS_DATA
            ? this._renderLoadingProgressView()
            : null}
          {this.state.viewStatus == ViewStatus.VIEW_STATUS_REQUEST_NETWORK_ERROR
  ...
  _renderDownloadMessage() {
    ...
    return (
      <Text
        style={{ fontSize: 14, color: Colors.fontBlack, paddingBottom: 35 }}
      >
        {message}
      </Text>
    );
  ...
  _renderLoadingProgressView() {
    return (
      <View>
        {Platform.OS == "ios" ? (
          <ProgressViewIOS
            progressTintColor={Colors.main_orange}
  ...
  _renderRetryView() {
    return (
      <Button
        style={{ height: 45, width: 80, backgroundColor: Colors.main_orange }}
        title="重试"
  ...
  _startDownload() {
    ...
    this._remotePackage
      .download(progress => this._downloadProgressHasChange(progress))
      .then(localPackage => {
