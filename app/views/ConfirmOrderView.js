...
class ConfirmOrderView extends Component {
  ...
  _createOrder() {
    ...
    let _placeId = this.props.currentPlace
      ? this.props.currentPlace.id || null
      : null
  ...
  _countTotal() {
    ...
    if (monthTicketAmount > 0 && isMonthTicketUsed) {
      // 证明有月票进行抵购或者月票是否可用
      _total =
        foodUnitPrice * (foodNum - 1) +
        (typeof addPrice != 'undefined' && addPrice ? addPrice : 0) // 月票抵购一份饭
    } else {
      //如果没有月票 就进行优惠券抵购
  ...
  _confirmOrderWithToken(token) {
    ...
    let placeId = isEmpty(this.props.currentPlace.id)
      ? null
  ...
  _currentPayType = () => {
    ...
    defaultPayment =
      hasChangeDefaultPayment != null ? hasChangeDefaultPayment : defaultPayment
    return EXPLAIN_PAY_TYPE[defaultPayment][this.props.language]
  }

  _renderMonthTicketView() {
    const {
      orderDetail: { monthTicketAmount },
      isMonthTicketUsed
    } = this.state
    return (
      <View
  ...
  _renderCouponView() {
    ...
    return (
      <View
        ...
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <Image
            ...
            style={{
              ...
              height: em(18),
              marginRight: em(11.5),
              marginTop: Platform.OS == 'android' ? em(2) : 0
            }}
          />
          <Text>
            {isMonthTicketUsed && monthTicketAmount
              ? `優惠券`
              : couponDetail
              ? `通用`
              : '不可用'}
        ...
        <Text>
          {isMonthTicketUsed && monthTicketAmount
            ? `${(couponDetail && couponDetail.totalAmount) || '0'}張`
            : couponDetail
            ? `- HKD ${parseInt(couponDetail.discount).toFixed(2)}`
            : '0張'}
        </Text>
      </View>
    )
  ...
  _renderNewCouponView() {
    ...
    return (
      <CommonItem
        ...
        rightIcon={
          <Icon name='ios-arrow-forward' style={ConfirmOrderStyles.ArrowShow} />
        }
        clickFunc={() =>
  ...
  _renderNewOrderView() {
    ...
    totalMoney =
      isMonthTicketUsed && monthTicketAmount
        ? totalMoney
        : totalMoney - discountsPrice > 0
        ? totalMoney - discountsPrice
        : 0
    return (
      <View style={styles.commonNewContainer}>
        <View style={[ConfirmOrderStyles.NewsInner, styles.commonMarginTop]}>
    ...
    return (
      <View
        style={[
          ...
          {
            marginBottom: GLOBAL_PARAMS.isIphoneX() ? em(100) : em(60)
          }
        ]}>
        <Text
          style={[
  ...
  _renderBottomConfirmView() {
    return (
      <View style={ConfirmOrderStyles.footer}>
        <View>
          <Text
            style={{ color: '#ff5050', fontSize: em(24), marginTop: em(-8) }}>
            HKD {this.state.orderDetail ? this._countTotal().toFixed(2) : '--'}
          </Text>
          <Text style={{ color: '#666', fontSize: em(14) }}>還需支付 </Text>
        </View>
        <CommonBottomBtn
          containerStyle={{
            marginTop: GLOBAL_PARAMS.isIphoneX() ? 0 : em(10),
  ...
  render() {
    let { i18n, isError, isExpired, expiredMessage } = this.state
