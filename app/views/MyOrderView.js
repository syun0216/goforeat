...
class MyOrderView extends Component {
  ...
  constructor(props) {
    super(props)
    this.state = {
      currentTab: _TAB_DELIVERING, // 当前的tab
      currentStatus: _ORDER_DELIVERING, // 当前的订单状态
      currentPickImage: null, //当前点击的地区图片
      currentPickTitle: '', // 当前点击的地区名称
      i18n: I18n[props.language], // 语言
      listCount: 0 //列表条数
    }
  }
  ...
  _renderNewListItemView(item, index) {
    return (
      <View style={{ padding: 20, backgroundColor: '#fff' }} key={index}>
        {this._renderOrderDetailsView(item)}
        {this._renderPayView(item)}
        {this._renderOperateBtnGroup(item)}
      </View>
    )
  }
  /**
  ...
  _renderOrderDetailsView(item) {
    ...
    return (
      <View
        style={[
          MyOrderStyles.FoodContainer,
          { height: item.addAmount ? em(120) : 110 }
        ]}>
        <FastImage
            ...
            <TouchableOpacity
              ...
              <Antd
                ...
                style={{
                  ...
                  marginTop: Platform.OS == 'ios' ? em(2) : em(4)
                }}
              />
            </TouchableOpacity>
          </View>
    ...
    return (
      <View style={MyOrderStyles.payContainer}>
        ...
        <View style={MyOrderStyles.payInner}>
          <View
            style={[
              MyOrderStyles.totalInnerView,
              item.status == _ORDER_ALL ||
                (item.status == _ORDER_CANCEL && {
                  position: 'absolute',
                  right: 0,
                  bottom: em(-20)
                })
  ...
  _renderPopupDialog() {
    return (
      <PreviewPlaceImg
        getRef={r => (this.popupDialog = r)}
        title={this.state.currentPickTitle}
        img={this.state.currentPickImage}
      />
    )
  }
  /**
  ...
  render() {
    ...
    return (
      <CustomizeContainer.SafeView
        ...
        <CommonComment
          ref={c => (this._commentView = c)}
          callback={() => {
            this[`${_TAB_COMMENT}flatlist`]
              .getWrappedInstance()
              .outSideRefresh()
